...
const Managers = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetchAPI('/admin/members');
      setUsers(response);
    } catch (error) {
  ...
  const fetchPosts = async () => {
    try {
      const data = await fetchAPI('/admin/qboards');
      // 데이터 구조 확인
  ...
  //   try {
  //     await Promise.all(selectedUsers.map(id => fetchAPI(`/admin/members/${id}`, { method: 'DELETE' })));
  ...
  const handleAddProduct = async () => {
    try {
      ...
      const addedProduct = await fetchAPI('/admin/products', {
        method: 'POST',
  ...
  const handleUpdateProduct = async () => {
    try {
      const response = await fetchAPI(`/admin/products/${products[editIndex].productId}`, {
        method: 'PUT',
  ...
  const handleDeleteProducts = async () => {
    try {
      await Promise.all(selectedProducts.map(index => fetchAPI(`/admin/products/${products[index].productId}`, { method: 'DELETE' })));
