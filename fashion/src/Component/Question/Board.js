...
const Board = () => {
  ...
  useEffect(() => {
    ...
    let url = localStorage.getItem('username') == 'admin' ? '/admin/qboards' : '/qboards/my'
          ...
          <tbody>
            {currentItems.map((item, index) => (
              ...
              >
                ...
                <td>
                {item.id === qnaData[0].id && (
                    <span style={{ color: 'red', fontWeight: 'bold' }}>[NEW] </span>
                  )}
                  {item.title}
                </td>
                <td>{localStorage.getItem('username') == 'admin' ?  item.username : item.username || '알 수 없음'}</td>
