        ...
        for jfile in tqdm(json_files, desc="Loading images and processing data"):
            ...
            if extracted is not None:
                ...
                if train:
                    ...
                    # image_path = list(glob.glob(f"examples/{ID}.jpg", recursive=True))
                    image_path = list(glob.glob(data_root + f"Validation/원천데이터/**/*/{ID}.jpg", recursive=True))
                assert len(image_path) == 1

                images.append(image_path[0])  # image_path는 리스트이므로 첫 번째 요소를 추가
...
def collate_fn(batch):
    ...
    for image_idx, image in enumerate(images):
        ...
        # ID 키가 존재하는지 확인하고, 없는 경우 오류 발생
        # if "ID" in targets[image_idx]:
        #     indices.append(targets[image_idx]["ID"])
        #     print(targets[image_idx]["ID"])
        #     del targets[image_idx]["ID"]
        # else:
        #     raise KeyError(f"'ID' key not found in targets for image index {image_idx}")
...
def load_data(train, batch_size=16, num_workers=0, data_root="./"):
    save_dir = './preprocessed_data/train'
