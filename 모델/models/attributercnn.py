...
class NewRoIHeadsAttributes(torch.nn.Module):
    def __init__(self, orh, num_classes, attribute_dict):
        ...
        self.box_predictor = FastRCNNAttributePredictor(inchannels, num_classes, attribute_dict) # 예측기 정의
    ...
    @property
    def has_keypoint(self): # keypoint가 있는지
    ...
    @property
    def has_match(self): #match 가 있는지
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, gt_attributes): 
        """
        1. 제안된 ROI(있을것이라는 가능성이 있는 영역)에 대해 GT레이블(정답)과 매칭
        2. GT레이블 및 속성 할당
        3. 배경 및 무시되는 RoI처리
        """
        matched_idxs = []
        labels = []
        attributes = []
        #porosals는 모델이 생성한 RoI, gt는 실제 객체의 박스
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_attributes_in_image in zip(proposals, gt_boxes, gt_labels, gt_attributes):
            ...
            #임계값 이하로 매칭된 RoI는 배경(0)으로 간주
            bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD
            labels_in_image[bg_inds] = 0
            #임계값 사이에 있으면 무시됨 무시는 -1
    ...
    def subsample(self, labels):
        ...
        for img_idx, (pos_inds_img, neg_inds_img) in enumerate(
            ...
            # 양성과 음성 인덱스를 결합하여 1차원 텐서로 반환
        return sampled_inds

    def add_gt_proposals(self, proposals, gt_boxes):
        """모델이 생성한 RoI와 실제 객체 상자를 결합하여 더 잘 학습할 수 있도록 함"""
        proposals = [
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.training:  # 훈련중이면 손실 계산
            ...
            for i in range(num_images): # 결과 구성
                if boxes[i].numel() > 0:
                    result.append(
                        dict(
                            boxes=boxes[i],
