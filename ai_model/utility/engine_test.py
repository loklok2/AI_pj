...
class Engine(object):
    ...
    def on_end_epoch(self, p, r):
        #예측 및 실제 결과를 numpy로 변경(텐서는 gpu로 돌려서 cpu로전환 한 뒤 numpy로 변경)
        ...
        print("s_id=",  s_id , "w_id", w_id)
        s = np.array(s_id)
        print("s : ", s , s.shape, type(s))
        w = np.array(w_id)
        print("w : ", w , w.shape, type(w))
        print("pp=", pp, pp.size, type(pp))
        # top-5 정답 뽑기
        l_r_s = real[s]
        l_p_s = pp[s]
        df = pd.DataFrame(np.array(img_name_list)[s])
    ...
    def learning(self, model, criterion, val_dataset):
        ...
        if self._state('resume') is not None:
            if os.path.isfile(self.state['resume']):
                ...
                # 쿠다 사용여부에 따라 모델 체크포인트 로드 방식 분류
                if torch.cuda.is_available():
                    checkpoint = torch.load(self.state['resume'], weights_only=True)
                else:
