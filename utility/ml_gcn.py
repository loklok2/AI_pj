...
class GCNResnet(nn.Module):
    ...
    def extract_features2(self, x, inp):
        feature = self.features(x)
        feature = self.pooling(feature)
        feature = feature.view(feature.size(0), -1)
...
# 특징 추출기 함수 추가
def get_feature_extractor(num_classes, t, pretrained=True, adj_file=None, in_channel=300):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = gcn_resnet101(num_classes, t, pretrained, adj_file, in_channel).to(device)
