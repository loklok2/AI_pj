...
public class StoresSalesController {
    ...
    public ResponseEntity<List<Map<String, Object>>> getTop5SellingProductsByStoreAndYear(
            @PathVariable("storeId") Long storeId,
            @PathVariable("year") int year) {
        return ResponseEntity.ok(storesSalesService.getTop5SellingProductsByStoreAndYear(storeId, year));
    }

    // 매장 매출 조회 년도 + 월별(개별 매장 리스트로 반환)
    @GetMapping("/top5/store/{storeId}/{year}/{month}")
    public ResponseEntity<List<Map<String, Object>>> getTop5SellingProductsByStoreAndYearAndMonth(
            @PathVariable("storeId") Long storeId,
            @PathVariable("year") int year,
            @PathVariable("month") int month) {
        return ResponseEntity.ok(storesSalesService.getTop5SellingProductsByStoreAndYearAndMonth(storeId, year, month));
    }

    // 매장 매출 조회 날짜별(개별 매장 리스트로 반환)
    @GetMapping("/top5/store/{storeId}/date/{date}")
    public ResponseEntity<List<Map<String, Object>>> getTop5SellingProductsByStoreAndDate(
            @PathVariable("storeId") Long storeId,
            @PathVariable("date") @DateTimeFormat(pattern = "yyyy-MM-dd") Date date) {
        return ResponseEntity.ok(storesSalesService.getTop5SellingProductsByStoreAndDate(storeId, date));
    }

    // 매장 매출 조회 년도 + 월 + 일별(개별 매장 리스트로 반환)
    @GetMapping("/top5/store/{storeId}/{year}/{month}/{day}")
    public ResponseEntity<List<Map<String, Object>>> getTop5SellingProductsByStoreAndYearMonthDay(
            @PathVariable("storeId") Long storeId,
            @PathVariable("year") int year,
            @PathVariable("month") int month,
            @PathVariable("day") int day) {
        return ResponseEntity
                .ok(storesSalesService.getTop5SellingProductsByStoreAndYearMonthDay(storeId, year, month, day));
    }

    // 매장 매출 조회 년도별(전체 매장 리스트로 반환)
    @GetMapping("/stores/{year}")
    public ResponseEntity<List<Map<String, Object>>> getSalesByStoreAndYear(@PathVariable("year") int year) {
    ...
    public ResponseEntity<List<Map<String, Object>>> getSalesByStoreAndYearAndMonth(
            @PathVariable("year") int year, @PathVariable("month") int month) {
        return ResponseEntity.ok(storesSalesService.getSalesByStoreAndYearAndMonth(year, month));
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> getSalesByStoreAndYearMonthDay(
            @PathVariable("year") int year,
            @PathVariable("month") int month,
            @PathVariable("day") int day) {
        return ResponseEntity.ok(storesSalesService.getSalesByStoreAndYearMonthDay(year, month, day));
    }
    ...
    public ResponseEntity<Map<String, Object>> getSalesByStoreIdAndYear(
            @PathVariable("storeId") Long storeId,
            @PathVariable("year") int year) {
        Map<String, Object> result = storesSalesService.getSalesByStoreIdAndYear(storeId, year);
        if (result == null) {
    ...
    public ResponseEntity<Map<String, Object>> getSalesByStoreIdAndYearAndMonth(
            ...
            @PathVariable("year") int year,
            @PathVariable("month") int month) {
        Map<String, Object> result = storesSalesService.getSalesByStoreIdAndYearAndMonth(storeId, year, month);
        if (result == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(result);
    ...
    public ResponseEntity<Map<String, Object>> getSalesByStoreIdAndDate(
            @PathVariable("storeId") Long storeId,
            @PathVariable("date") @DateTimeFormat(pattern = "yyyy-MM-dd") Date date) {
        Map<String, Object> result = storesSalesService.getSalesByStoreIdAndDate(storeId, date);
        if (result == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(result);
    ...
    public ResponseEntity<Map<String, Object>> getSalesByStoreIdAndYearMonthDay(
            ...
            @PathVariable("month") int month,
            @PathVariable("day") int day) {
        Map<String, Object> result = storesSalesService.getSalesByStoreIdAndYearMonthDay(storeId, year, month, day);
        if (result == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(result);
